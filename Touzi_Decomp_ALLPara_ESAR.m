function [DATA_Feat, DATA_Feat_order]=Touzi_Decomp_ALLPara_ESAR(location,r,c)
% % % % clc;
% % % % clear;
% % % % close all;
...
%% obtain Lambda1 Lambda2 Lambda3
file_id = fopen(strcat(location, 'l1.bin'),'rb');
...
file_id = fopen(strcat(location, 'l2.bin'),'rb');
status=fseek(file_id,0,'bof');
for n=1:r
    pix=fread(file_id,c,'float32');
    Lambda2(n,:)=pix;
...
file_id = fopen(strcat(location, 'l3.bin'),'rb');
status=fseek(file_id,0,'bof');
for n=1:r
    pix=fread(file_id,c,'float32');
    Lambda3(n,:)=pix;
...
Lam1_DATA=reshape(Lambda1,1,r*c);
Lam2_DATA=reshape(Lambda2,1,r*c);
Lam3_DATA=reshape(Lambda3,1,r*c);
SPAN_DATA=reshape(SPAN,1,r*c);
...
DATA_Feat=[Lam1_DATA;Lam2_DATA;Lam3_DATA;SPAN_DATA;ALP1_DATA;ALP2_DATA;ALP3_DATA;PHI1_DATA;PHI2_DATA;PHI3_DATA;
    PSI1_DATA;PSI2_DATA;PSI3_DATA;TAU1_DATA;TAU2_DATA;TAU3_DATA;];



DATA_Feat_order=[Lam1_DATA;Lam2_DATA;Lam3_DATA;SPAN_DATA;ALP1_DATA;ALP2_DATA;ALP3_DATA;PHI1_DATA;PHI2_DATA;PHI3_DATA;
