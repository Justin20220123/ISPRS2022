...
function [pred1,pred2,acc1,acc2,pred_Unlabeled_BF,pred_Unlabeled_AF,testSet,unlabeledSet,testLabels,tr_Time_NF, tr_Time_WF,pr_Time_NF,pr_Time_WF]=AE_Softmax_FinetuneESAR(inputSize,hiddenSizeL1,numClasses,...
...
[sae1OptTheta, cost] = minFunc( @(p) sparseAutoencoderCost(p,inputSize, hiddenSizeL1,lambda, sparsityParam1,beta,b), ...
                              sae1Theta, options);
...
tic
[pred_Unlabeled_BF] = stackedAE2Predict(stackedAETheta, inputSize,hiddenSizeL1, numClasses,unlabeledData );
[pred1] = stackedAE2Predict(stackedAETheta, inputSize,hiddenSizeL1, numClasses, testData);
pr_Time_NF=toc
acc1 = mean(testLabels(:) == pred1(:));
fprintf('Before Finetuning Test Accuracy: %0.3f%%\n', acc1 * 100);

%% predcit all the pixels in the images
tic
[pred_Unlabeled_AF] = stackedAE2Predict(stackedAEOptTheta, inputSize,hiddenSizeL1, numClasses,unlabeledData );
